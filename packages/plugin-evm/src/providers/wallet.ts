...
export class WalletProvider {
    ...
    async getERC20Balance(
        ...
        const cacheKey = `erc20Balance_${chainName}_${tokenAddress}`;
        const cachedData = await this.getCachedData<string>(cacheKey);

        if (cachedData) {
            elizaLogger.log(
                `Returning cached ERC-20 balance for token: ${tokenAddress}, chain: ${chainName}`
            );
            return cachedData;
        }
        ...
        try {
            ...
            const formattedBalance = formatUnits(balance, decimals); // Assuming 18 decimals
            this.setCachedData<string>(cacheKey, formattedBalance);

            elizaLogger.log(
                `ERC-20 balance cached for token: ${tokenAddress}, chain: ${chainName}`
            );
            ...
            elizaLogger.log(
                `ERC-20 token symbol: ${symbol}, balance: ${formattedBalance}`
            );
            return formattedBalance;
        } catch (error) {
            console.error(
                `Error fetching ERC-20 balance for token: ${tokenAddress}, chain: ${chainName}`,
                error
            );
            return null;
        }
    ...
    async getBalancesForTokens(
        ...
        const balances: Record<Address, string | null> = {};
        for (const tokenAddress of tokenAddresses) {
            balances[tokenAddress] = await this.getERC20Balance(
                tokenAddress,
                chainName
            );
        }
    ...
    async fetchTokensForAddress(
        runtime: IAgentRuntime,
        walletAddress?: Address
    ): Promise<Address[]> {
        const etherscanApiKey = runtime.getSetting("ETHERSCAN_API_KEY");
        if (!etherscanApiKey) {
            throw new Error("ETHERSCAN_API_KEY is missing in runtime settings");
        }
        ...
        if (data.status !== "1" || !data.result) {
            throw new Error(`Failed to fetch tokens for address: ${address}`);
        }
        if (!data.result) {
            console.error("Unexpected response from Etherscan API:", data);
            throw new Error("Failed to fetch token list");
        }

        return data.result.map(
            (token: { contractAddress: Address }) => token.contractAddress
        );
    }
